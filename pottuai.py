...
class GeminiOracle:
    ...
    def build_private_path_image(
        ...
        """
        PRIVATE ONLY.
        This image is never displayed in the Qt result UI.
    ...
    def _worker(
        ...
        try:
            ...
            ok_clean, clean_jpg = cv2.imencode(
                ".jpg",
                clean_frame,
                [
                    int(
...
നിങ്ങൾക്ക് രണ്ട് ചിത്രങ്ങൾ ലഭിക്കുന്നു.

ആദ്യ ചിത്രം:
ടാർഗറ്റ് എത്തിയ നിമിഷത്തിലെ clean final camera frame.

രണ്ടാമത്തെ PRIVATE analysis image:
...
നിങ്ങളുടെ ജോലി:
കളിക്കാരന്റെ കൈയാത്രയുടെ shape യഥാർത്ഥത്തിൽ പരിശോധിക്കുക.
അത് നേരെയാണോ, വളഞ്ഞതാണോ, shaky ആണോ, zig-zag ആണോ,
വലിയ detour ഉണ്ടോ, അവസാനത്തിൽ panic correction ഉണ്ടോ,
അല്ലെങ്കിൽ വളരെ controlled ആണോ എന്ന് വിലയിരുത്തുക.

Horoscope output screenshot-style ആയിരിക്കണം:
- മലയാളത്തിൽ മാത്രം.
...
- King Mahabali / മഹാബലി സ്വാഭാവികമായി ഉൾപ്പെടുത്തുക.
- ഓരോ റൗണ്ടിലും wording വ്യത്യസ്തമാക്കുക.
- previous attempt context ഉണ്ടെങ്കിൽ പ്രസക്തമായ ഒരു മാറ്റം മാത്രം
  സ്വാഭാവികമായി പരാമർശിക്കാം.
- telemetryയ്ക്ക് വിരുദ്ധമായ claim പറയരുത്.
- exact numerical measurements horoscopeൽ പറയേണ്ടതില്ല.
- "AI", "Gemini", "pixel", "telemetry" എന്നീ technical words ഉപയോഗിക്കരുത്.
- യഥാർത്ഥ supernatural certainty claim ചെയ്യരുത്.
- movement patternനോട് നേരിട്ട് ബന്ധപ്പെട്ട ഒരു fun future-style prediction നൽകുക.

Output:
മലയാളം horoscope paragraph മാത്രം.
""".strip()
                ...
                client.models.generate_content(
                    ...
                    contents=[
                        ...
                        types.Part.from_bytes(
                            data=clean_jpg.tobytes(),
                            mime_type="image/jpeg",
                        ),
                        types.Part.from_bytes(
                            data=path_jpg.tobytes(),
                            mime_type="image/jpeg",
                        ),
                    ],
                    config=types.GenerateContentConfig(
                        # This request has no tools/functions.
                        # Disable AFC explicitly to avoid the SDK warning:
            ...
            # UI adds the quote marks itself.
            text = (
                text
                .strip()
                .strip('"')
            ...
            # Same final Malayalam paragraph is read aloud.
            ...
            # Even if Gemini 3.7 generation fails, try to read the
            # deterministic Malayalam fallback through Gemini Live.
            live_voice.speak_oracle(
                fallback
            )
...
def main():
    ...
    try:
        while True:
            ...
            # Start Gemini 3.7 analysis immediately.
            oracle.start(
